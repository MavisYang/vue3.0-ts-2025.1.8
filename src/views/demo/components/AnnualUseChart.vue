...
const data = reactive({
	...
	unit: annualData.value ? annualData.value.map((val) => val.label) : [],
...
watch(
	() => props.echartData,
	(newVal) => {
		annualData.value = newVal;
		data.data = newVal ?? [];
		data.unit = newVal ? newVal.map((val) => val.label) : [];

		option.series = data.data.map((val: ChartProp, index: number) => {
			return {
				name: val.label + "年",
				type: "line",
				symbol: "circle",
